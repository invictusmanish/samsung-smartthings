...
import {
  CARLITA_DATA_MUSICFRAME,
  CARLITA_DATA_SAMRTPLUG,
  CARLITA_DATA_SERO,
  CARLITA_DATA_WATCH,
  CARLITA_DATA_ZFLIP,
} from '../../../components/Layout/Carlita.data';
...
function Carlita() {
  ...
  const [galaxyTitle, setGalaxyTitle] = useState(
    CARLITA_DATA_WATCH.galaxyDetails[0]?.title,
  );
  const [zFlipDescription, setZflipDescription] = useState(
    CARLITA_DATA_ZFLIP.galaxyZFlipDetails[0]?.description,
  );
        ...
        >
          {isHotspot && hotSpotValue == 'Smart Plug' ? (
            <div>
              <div>
                <Text className="font-bold text-center">
                  {CARLITA_DATA_SAMRTPLUG.title}
                </Text>
              </div>
              <div>
                <Text as="title">{CARLITA_DATA_SAMRTPLUG.caption}</Text>
              </div>
              <div className="py-4 px-4 flex justify-center items-center">
                <img
                  src={CARLITA_DATA_SAMRTPLUG.image}
                  alt="Smart Plug"
                />
              </div>
              <div>
                <Text className="font-sans text-center">
                  {CARLITA_DATA_SAMRTPLUG.description}
                </Text>
              </div>
            </div>
          ) : hotSpotValue == 'Sero' ? (
            <div>
              <div>
                <Text className="font-bold text-center">
                  {CARLITA_DATA_SERO.title}
                </Text>
              </div>
              <div>
                <Text as="title">{CARLITA_DATA_SERO.caption}</Text>
              </div>
              <div className="py-0 px-2">
                <ul>
                  {CARLITA_DATA_SERO.description.map((item, index) => (
                    <li key={index}>
            ...
            <div>
              <div>
                <Text className="font-bold text-center text-xs">
                  {CARLITA_DATA_MUSICFRAME.title}
                </Text>
              </div>
              <div>
                <Text as="caption">{CARLITA_DATA_MUSICFRAME.caption}</Text>
              </div>
              <div className="py-4 px-4 grid grid-cols-2 items-center">
                ...
                <div className="ps-8">
                  <ul>
                    {CARLITA_DATA_MUSICFRAME.description.map((item, index) => (
                      <li key={index}>
                        <p className="text-xs text-left">{`\u2981 ${item}`}</p>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="py-4 px-4  flex justify-between">
                {CARLITA_DATA_WATCH.galaxyDetails.map((item, index) => (
                  <img
                    ...
                    alt="image"
                    onClick={() => {
                      setGalaxyDescription(item?.description);
                      setGalaxyTitle(item?.title);
                    }}
                  />
                ))}
              </div>
              ...
              <div className="py-4 px-4  flex justify-between">
                {CARLITA_DATA_ZFLIP.galaxyZFlipDetails.map((item, index) => (
                  <img
                    ...
                    alt="image"
                    onClick={() => {
                      setZflipDescription(item?.description);
                      setZflipTitle(item?.title);
                    }}
                  />
                ))}
              </div>
              <div className="py-1 px-2 flex justify-content:center">
                {CARLITA_DATA_ZFLIP.galaxyZFlipDetails.map((item, index) => (
                  <Text className="font-bold text-center text-xs" key={index}>{item?.text}</Text>
                ))}
              </div>
            ...
            <>
              <div className="py-4 px-4 flex justify-center items-center"></div>
              <img
                className="mx-auto"
                src={UpArrow}
                alt={'Arrow'}
